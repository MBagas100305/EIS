...
export default function RingkasanKinerjaOperasiPage(): React.ReactElement {
  ...
  const indicators = [
    ...
    "Air Baku",
    "Listrik",
    "Air Produksi Tahun Berjalan",
    "Losses Distribusi  Tahun Berjalan",
    "Losses Produksi Tahun Berjalan",
    "Volume Penjualan Tahun Berjalan",
    "Air Baku Tahun Berjalan",
  ...
  // ✅ Filtered data untuk KPI summary
  ...
  // KPI summary
  const totalActualAll = filteredCutoffData.reduce((s, r) => s + (Number(r.ActualNumber || 0) || 0), 0);
  const totalRKAPAll = filteredCutoffData.reduce((s, r) => s + (Number(r.RKAPNumber || 0) || 0), 0);
    ...
    <main className="min-h-screen bg-slate-50 p-10">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-8
        ...
        <div>
          <h1 className="text-4xl font-extrabold text-slate-800">
            Ringkasan Kinerja Operasi <span className="text-blue-600">{selectedYearLabel}</span>
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {filteredIndicators.map((indicator) => {
          ...
          const totalActual = chartData.reduce((sum, item) => sum + (item.currentYear || 0), 0);
          const totalRKAP = chartData.reduce((sum, item) => sum + (item.lastYear || 0), 0);
                ...
                <CardTitle className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="text-xs text-slate-500 mt-1">
                      Satuan: {typedData.find((r) => r.Indicator === indicator)?.Unit ?? "M³"}
                    </div>
                  </div>
                </CardTitle>
              </CardHeader>
